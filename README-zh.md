...
**评估者是智能体（Agent），合议是技能（Skill）。** 13位评估者是以人格（persona）为基础的专业智能体（`agents/{name}.md`），在各自独立的上下文中进行评估。技能会共享同一个上下文，因此不适合进行独立评估；通过以子智能体（subagent）方式启动，可以使上下文彼此隔离。唯一的技能是合议编排器（story-council）。
...
| **3** | **合议（full）** | 填满全部 13 个维度的完整 Story Report | 从一开始就让所有人一次性评估 |
...
| **召集范围** `mode` | `auto`（默认）/ `full` | `auto` 依据领域召集 **3～5 位**，`full` 召集适用范围内的**全体**（text：13 位 / plot：10 位） |
...
13 位评估者都是**子智能体**（`agents/{name}.md`）。每位评估者只对自己的维度打分，并与相邻维度保持明确的边界。
...
2. 系统以 13 位评估者、依据**结构性标准**（第二重盲检）进行评估，并推导出分类
...
- **Phase 1**：Novel Evaluation Core——13 位评估者智能体（以结构性维度与权重实现）+ 双重盲检（anonymize + 结构性校准）+ 合议技能（story-council）+ Story Vector + Story Report
