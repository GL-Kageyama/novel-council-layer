#!/usr/bin/env python3
"""Validate a Novel Council evaluator output against the shared JSON schema.
...
SCHEMA_PATH = os.path.join(
    os.path.dirname(__file__), "..", "schemas", "novel-value-output.schema.json"
)
...
DOMAINS = ["pure-literature", "genre-fiction", "light-novel", "short-story", "historical-fiction"]
...
def validate_report(obj):
    """Validate a council Story Report (produced by skills/story-council/SKILL.md)."""
    errors = []
    ...
    if "non_consulted_evaluators" in obj:
        nc = obj["non_consulted_evaluators"]
        if not isinstance(nc, list):
            errors.append("non_consulted_evaluators must be an array")
        else:
    ...
    if "story_vector" in obj:
        ...
        if not isinstance(vec, dict):
            errors.append("story_vector must be an object")
        else:
            unknown = set(vec.keys()) - set(VECTOR_DIMENSIONS)
            if unknown:
                errors.append(f"story_vector has unknown dimensions: {sorted(unknown)}")
