...
- `agents/{name}.md` — 評価者エージェントの正本（12体）。ペルソナベースの専門家として独立したサブエージェントで起動される
...
`content_type: "plot"` を指定すると、執筆前の構想・簡単なあらすじでも評価できる。散文・語り・読書体験が存在しないため、`prose-style`, `narrative-technique`, `reader-experience` の3体は未招集となり、残り9体（narrative-originality, anti-generic-story-filter, emotional-power, plot-architecture, character-depth, theme-resonance, world-building, admiration, hook）で評価する。
...
| `utils/render_report.py` | Story Report の視覚表示（12次元バーチャート・分類バッジ・次元間の対立）。`-o report.md` でMarkdown文書として保存、`--individuals` で全個別レポート表示 |
