...
- `agents/{name}.md` — 评价者代理的正本（10位）。作为基于人格的专家，以独立子代理启动
...
指定 `content_type: "plot"`，即可对执笔前的构想或简单梗概进行评价。由于不存在散文、叙事与阅读体验，`prose-style`、`narrative-technique`、`reader-experience` 这3位不被召集，由其余7位（narrative-originality, anti-generic-story-filter, emotional-power, plot-architecture, character-depth, theme-resonance, world-building）进行评价。
...
| `utils/render_report.py` | Story Report的视觉展示（10维条形图・分类徽章・维度间的对立）。用`-o report.md`保存为Markdown文档，用`--individuals`显示全部个别报告 |
